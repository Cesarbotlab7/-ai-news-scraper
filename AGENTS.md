...
- `web/` — Next.js 16前端，本地已跑通，待部署腾讯云上海
...
- **Next.js 16**，`params` 和 `searchParams` 都是 `Promise`，必须 `await` 才能解构
- **Supabase anon key + RLS**（`SELECT` 对 anon 全开，写入仅 service_role）
- **ISR**：首页/详情页均 `export const revalidate = 900`（15分钟）
- **Twitter content 两个 fallback**：`title` 为空 → `stripMarkdown(content).slice(0,120)`；`published_at` 为 null → `fetched_at`
- **Markdown 清洗**：Jina 返回的 content 含原始 Markdown，`lib/format.ts` 的 `stripMarkdown()` 在卡片展示前处理

页面路由：
```
/                      首页信息流（is_representative=true，按 importance_score 降序）
/?tab=twitter          信源过滤（twitter/rss/hackernews/arxiv），server component 读 searchParams
/news/[url_hash]       详情页（完整 content + cluster 兄弟条目）
```
...
**事件聚合**：`cluster.py` 生成embedding → 余弦相似度矩阵 → 并查集合并。`DASHSCOPE_API_KEY` 未设置时静默跳过（不报错）。`is_representative=true` 的条目才在前端信息流展示。`cluster_count` 是总条目数（含代表条目），前端展示「另有 N-1 个源」。
...
| `SUPABASE_SERVICE_KEY` | Service Role Key（绕过RLS，仅 scraper 用） |
| `DASHSCOPE_API_KEY` | 向量嵌入 + AI摘要（qwen-turbo） |
| `JINA_API_KEY` | s.jina.ai搜索 + r.jina.ai正文（必填，否则401）|

Web 前端环境变量（`web/.env.local`，已在 .gitignore）：
